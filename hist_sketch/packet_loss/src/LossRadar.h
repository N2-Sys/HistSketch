...
public:
	new_data_t flowID;
	uint32_t IP_ID;
	char str[CHARKEY_LEN + 5];

	CustomizedKey(new_data_t k, uint32_t ip_id):flowID(k), IP_ID(ip_id) {
...
public:
	...
	std::vector<CustomizedKey> lost_packet;
	...
	uint32_t hash(Key_t key, int i) {
		return (uint32_t)(AwareHash((unsigned char *)key, CHARKEY_LEN + 5, h[i], s[i], n[i]) % size);
	}

	void insert(const CustomizedKey &k) {
		std::set<uint32_t> pos_set;
		for (int i = 0; i < hash_num; ++i) {
			pos_set.insert(hash((Key_t)k.str, i));
		}
			...
			for (int i = 0; i < size; ++i) {
				if (packet_sum[i] == 1) {
					...
					for (int j = 0; j < hash_num; ++j) {
						pos_set.insert(hash((Key_t)xor_sum[i].str, j));
					}
					for (int i = 0; pos_set.size() < hash_num; ++i) {
						pos_set.insert((*pos_set.begin() + i) % size);
					}
					...
					lost_packet.push_back(xor_sum[i]);
					xor_sum[i] ^= xor_sum[i];
					packet_sum[i]--;
	...
	size_t get_memory_usage() {
		return (CHARKEY_LEN + 5 + sizeof(T)) * size;
	}
};
