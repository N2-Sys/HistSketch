...
double packet_loss_large(std::map<five_tuple, std::array<uint32_t, BUCKET_NUM>> &result, std::map<five_tuple, std::map<uint8_t, uint32_t>> & ground_truth_loss) {
	uint32_t tp = 0, fp = 0, tn = 0, fn = 0;
	for (auto it = result.begin(); it != result.end(); ++it) {
		bool is_loss = false;
		for (int i = 0; i < BUCKET_NUM; ++i) {
			if (i >= BIG_BORDER && it->second.at(i) != 0) {
				is_loss = true;
				break;
			}
		}
		bool is_loss_truth = (ground_truth_loss.find(it->first) != ground_truth_loss.end());
		if (is_loss) {
			if (is_loss_truth) {
				tp++;
			}
...
double packet_loss_small(std::map<five_tuple, std::array<uint32_t, BUCKET_NUM>> &result, std::map<five_tuple, std::map<uint8_t, uint32_t>> & ground_truth_loss) {
	uint32_t tp = 0, fp = 0, tn = 0, fn = 0;
	for (auto it = result.begin(); it != result.end(); ++it) {
		bool is_loss = false, small_non_zero = false, big_non_zero = false;
		for (int i = 0; i < BUCKET_NUM; ++i) {
			if (it->second.at(i) != 0) {
				if (i <= SMALL_BORDER) {
					small_non_zero = true;
		...
		is_loss = small_non_zero & !big_non_zero;
		bool is_loss_truth = (ground_truth_loss.find(it->first) != ground_truth_loss.end());
		if (is_loss) {
			if (is_loss_truth) {
				tp++;
...
double packet_loss_random(std::map<five_tuple, std::array<uint32_t, BUCKET_NUM>> &result, std::map<five_tuple, std::map<uint8_t, uint32_t>> & ground_truth_loss) {
	uint32_t tp = 0, fp = 0, tn = 0, fn = 0;
	for (auto it = result.begin(); it != result.end(); ++it) {
		bool is_loss = false;
		int non_zero = 0;
		...
		is_loss = (non_zero >= RANDOM_BORDER);
		bool is_loss_truth = (ground_truth_loss.find(it->first) != ground_truth_loss.end());
		if (is_loss) {
			if (is_loss_truth) {
				tp++;
