...
public:
	...
	int insert(uint16_t bid, uint32_t v = 1, bucket<T> &b = bucket<T>(), bool debug = false) {		// b for evicting a bucket
		bucket<T> temp;
		...
		for (int i = 0; i < HashNum; ++i) {
			pos = i * (bucket_num / HashNum) + (uint32_t)(AwareHash((unsigned char *)&bid, 2, h[i], s[i], n[i]) % (bucket_num / HashNum));
			// if (!i) temp_pos = pos;
	...
	T query(uint16_t bid, uint16_t &swap) {
		T result = 0;
		swap = 0;

		// uint32_t pos1 = (bucket_num / HashNum) + (uint32_t)(AwareHash((unsigned char *)&bid, 1, h[0], s[0], n[0]) % (bucket_num / HashNum));
		uint32_t pos1 = (uint32_t)(AwareHash((unsigned char *)&bid, 2, h[0], s[0], n[0]) % (bucket_num / HashNum));
		uint32_t pos2 = (bucket_num / HashNum) + (uint32_t)(AwareHash((unsigned char *)&bid, 2, h[1], s[1], n[1]) % (bucket_num / HashNum));
