...
public:
    BloomFilter();
    ~BloomFilter();
    bool getbit(CharKey_t k);
    void setbit(CharKey_t k);
    void reset();
...
template<uint32_t w, int hash_num, int key_len>
bool BloomFilter<w, hash_num, key_len>::getbit(CharKey_t k) {
    for (int i = 0; i < hash_num; ++i) {
...
template<uint32_t w, int hash_num, int key_len>
void BloomFilter<w, hash_num, key_len>::setbit(CharKey_t k) {
    for (int i = 0; i < hash_num; ++i) {
        uint32_t pos = (uint32_t)(AwareHash((unsigned char *)k, key_len, h[i], s[i], n[i]) % width);
