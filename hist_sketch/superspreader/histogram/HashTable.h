...
public:
	...
	static size_t get_memory_usage() {
		return sizeof(T) + INTKEY_LEN + histogram<T, bucket_num>::get_memory_usage();
	}
};
...
public:
	...
	uint32_t calPos(Key_t key) {
		return (uint32_t)(AwareHash((unsigned char *)&key, INTKEY_LEN, h, s, n) % slot_num);
	}

	int insert(Key_t k, uint16_t bid, new_int_data_t &swap_key, T &swap_val, slot<T, bucket_num> &swap_slot,
				uint32_t v = 1, bool debug = false)					// v: number of packets / pakcet length
	{
		...
		if (slots[pos].key == k) {
			...
			if (ret == HIST_EVICT) {									// evict a bucket of the histogram
				swap_key = new_int_data_t(k, b.idx);
				swap_val = b.val;
				// swap = 1;
				if (debug)
					std::cout << "HIT_EVICT\n";
	...
	T query(Key_t k, uint16_t bid, uint16_t &swap) {
		uint32_t pos = calPos(k);
		swap = 0;
