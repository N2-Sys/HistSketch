...
class PRSketch {
	private:
		...
		std::vector<five_tuple> flowkeyStorage;
		std::map<five_tuple, T> mp;
		bool decoded = false;

		uint32_t hash(unsigned char *key, int idx) {
			return (uint32_t)(AwareHash(key, CHARKEY_LEN, h[idx], s[idx], n[idx]) % width);
		}
	...
	public:
		...
		void insert(Key_t key, uint32_t val = 1) {	//note: flow ID + bucket ID
			...
			if (!bf.getbit(key)) {
				bf.setbit(key);
				flowkeyStorage.push_back(five_tuple((char *)key));
				bandwidth += CHARKEY_LEN + 1;
			}
			for (int i = 0; i < depth; ++i) {
				pos = (uint32_t)(AwareHash((unsigned char *)key, CHARKEY_LEN, h[i], s[i], n[i]) % width);
				matrix[i][pos] += val;
			}
		...
		void solve_equations(std::map<five_tuple, T> & mp) {
			if (decoded)
				return;
			...
			for (int i = 0, j; i < depth; i++) {
				...
				for (auto it = flowkeyStorage.begin(); it != flowkeyStorage.end(); ++it, ++j) {
					int idx = i * width + this->hash((unsigned char *)(it->str), i);
					//按Triplet方式填充，速度快
