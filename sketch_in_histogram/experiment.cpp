...
int main (int argc, char *argv[]) {
	...
	for (int i = 1; i < argc; ++i) {
		if (string(argv[i]) == "CM")
			testVector.push_back(new Histogram<CMSketch<uint32_t, CM_DEPTH, CM_WIDTH>, BUCKET_NUM, BUCKET_WIDTH>("CM"));
		else if (string(argv[i]) == "CU")
			testVector.push_back(new Histogram<CUSketch<uint32_t, CU_DEPTH, CU_WIDTH>, BUCKET_NUM, BUCKET_WIDTH>("CU"));
		else if (string(argv[i]) == "IBLT")
			testVector.push_back(new Histogram<IBLT<uint32_t, BF_WIDTH, BF_HASHNUM, IBLT_WIDTH, IBLT_HASHNUM>, BUCKET_NUM, BUCKET_WIDTH>("IBLT"));
		else if (string(argv[i]) == "CS")
			testVector.push_back(new Histogram<CountSketch<int32_t, CS_DEPTH, CS_WIDTH>, BUCKET_NUM, BUCKET_WIDTH>("CS"));
		else if (string(argv[i]) == "SuMax")
			testVector.push_back(new Histogram<SuMaxSketch<uint32_t, SM_DEPTH, SM_WIDTH>, BUCKET_NUM, BUCKET_WIDTH>("SuMax"));
		else if (string(argv[i]) == "ES")
			testVector.push_back(new Histogram<ElasticSketch<uint32_t, SLOT_NUM, LIGHT_DEPTH, LIGHT_WIDTH>, BUCKET_NUM, BUCKET_WIDTH>("ES"));
		else if (string(argv[i]) == "CBF")
			testVector.push_back(new Histogram<CBF<uint32_t, CBF_WIDTH, CBF_HASHNUM>, BUCKET_NUM, BUCKET_WIDTH>("CBF"));
		else if (string(argv[i]) == "NS")
			testVector.push_back(new Histogram<NitroSketch<int32_t, NS_DEPTH, NS_WIDTH>, BUCKET_NUM, BUCKET_WIDTH>("NS"));
		else if (string(argv[i]) == "UM")
			testVector.push_back(new Histogram<UnivMon<int32_t, UM_LAYER, UM_DEPTH, UM_WIDTH>, BUCKET_NUM, BUCKET_WIDTH>("UM"));
		else if (string(argv[i]) == "CCS")
			testVector.push_back(new Histogram<CoCoSketch<uint32_t, CCS_DEPTH, CCS_WIDTH>, BUCKET_NUM, BUCKET_WIDTH>("CCS"));
	}
