...
const uint32_t flow_number = 50000;
...
int main (int argc, char *argv[]) {
	...
	for (int i = 1; i < argc; ++i) {
		if (string(argv[i]) == "CM")
			testVector.push_back(new Histogram<CMSketch<uint16_t, CM_DEPTH, CM_WIDTH>, BUCKET_NUM, BUCKET_WIDTH>("CM"));
		else if (string(argv[i]) == "CU")
			testVector.push_back(new Histogram<CUSketch<uint16_t, CU_DEPTH, CU_WIDTH>, BUCKET_NUM, BUCKET_WIDTH>("CU"));
		else if (string(argv[i]) == "IBLT")
			testVector.push_back(new Histogram<IBLT<uint16_t, BF_WIDTH, BF_HASHNUM, IBLT_WIDTH, IBLT_HASHNUM>, BUCKET_NUM, BUCKET_WIDTH>("IBLT"));
		else if (string(argv[i]) == "CS")
			testVector.push_back(new Histogram<CountSketch<int16_t, CS_DEPTH, CS_WIDTH>, BUCKET_NUM, BUCKET_WIDTH>("CS"));
		else if (string(argv[i]) == "SuMax")
			testVector.push_back(new Histogram<SuMaxSketch<uint16_t, SM_DEPTH, SM_WIDTH>, BUCKET_NUM, BUCKET_WIDTH>("SuMax"));
		else if (string(argv[i]) == "ES")
			testVector.push_back(new Histogram<ElasticSketch<uint16_t, SLOT_NUM, LIGHT_DEPTH, LIGHT_WIDTH>, BUCKET_NUM, BUCKET_WIDTH>("ES"));
		else if (string(argv[i]) == "CBF")
			testVector.push_back(new Histogram<CBF<uint16_t, CBF_WIDTH, CBF_HASHNUM>, BUCKET_NUM, BUCKET_WIDTH>("CBF"));
		else if (string(argv[i]) == "NS")
			testVector.push_back(new Histogram<NitroSketch<int16_t, NS_DEPTH, NS_WIDTH>, BUCKET_NUM, BUCKET_WIDTH>("NS"));
		else if (string(argv[i]) == "UM")
			testVector.push_back(new Histogram<UnivMon<int16_t, UM_LAYER, UM_DEPTH, UM_WIDTH>, BUCKET_NUM, BUCKET_WIDTH>("UM"));
		else if (string(argv[i]) == "CCS")
			testVector.push_back(new Histogram<CoCoSketch<uint16_t, CCS_DEPTH, CCS_WIDTH>, BUCKET_NUM, BUCKET_WIDTH>("CCS"));
		else if (string(argv[i]) == "PR")
			testVector.push_back(new Histogram<PRSketch<uint16_t, PR_DEPTH, PR_WIDTH>, BUCKET_NUM, BUCKET_WIDTH>("PR"));
	}
